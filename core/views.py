...
def atividade_cadastro(request):
    ...
    # Testando se a ação recebida foi de salvar os dados e se o
    # formulário da atividade é válido
    if acao == 'salvar' and atividade_form.is_valid():
        # Salvando os dados do formulário da atividade e
        # guardando o objeto criado em "atividade"
        atividade = atividade_form.save()
        # A função zip permite iterar sobre as duas listas ao mesmo tempo
        for questao_form, alternativas_forms in zip(questoes_forms,
                                                    alternativas_forms_questao):
            if questao_form.is_valid():
                # Commit = false não deixa salvar os dados no banco, mas
                # retorna um objeto
    ...
    # Envia-se como contexto o formulário da atividade, a lista com os
    # formulários das questões e a lista com as listas dos formulários
    # das alternativas de cada questão
    contexto = {
